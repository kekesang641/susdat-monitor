...
# 定时播报：每 N 次运行播报一次（每次5分钟，3次=15分钟）
...
#  Gist 状态持久化
# ============================================================
SCRIPT_VERSION = "2.3"   # 版本变更时自动重置状态并发送启动通知
...
def main():
    ...
    try:
        ...
        print(f"Gist loaded OK | version={state.get('version')} | run_count={state.get('run_count')} | first_run={is_first_run}")
    except Exception as e:
    ...
    if is_first_run:
        ...
        send_tg(
            ...
            f"⚠️ 脱锚预警: SUSDAT &lt;${SUSDAT_DEPEG_LOW} 或 &gt;${SUSDAT_DEPEG_HIGH}\n"
            f"⏱ {now()}"
        )
        return
    ...
    if run_count % BROADCAST_EVERY_N == 0:
        ...
        strc_broadcast = f"{e2} STRC: <b>{fmt(strc_price)}</b>  ({strc_chg_30:+.2f}% / 15min)\n" if strc_price else "📈 STRC: 美股休市中\n"
        send_tg(
            f"📊 <b>双资产定时播报</b>\n\n"
            f"{e1} SUSDAT: <b>{fmt(susdat_price)}</b>  ({susdat_chg_30:+.3f}% / 15min)\n"
            f"   流动性: ${float(susdat_info.get('liquidity',0)):,.0f}\n\n"
            f"{strc_broadcast}"
            f"\n⏱ {now()}"
        )
